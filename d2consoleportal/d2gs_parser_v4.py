...
# --- Функция за Логване на Сурови Данни (НОВА) ---
...
# --- Основна функция за парсване ---

def parse_server_status():
    """Connects to the D2GS server, executes commands, and returns parsed data."""
    ...
    try:
        ...
        if not status_raw or not uptime_raw:
             ...
             log_raw_data(uptime_raw, status_raw) # Логваме и непълните данни
             return {"status": "error", "message": results["message"], "uptime": {}, "status_data": {}}
        ...
        if uptime_duration_match:
            ...
            # ... (Калкулации за total_seconds - както преди) ...
        ...
        # --- Status Parsing: Game/User Stats, Resources, Net Stats (Остава същото) ---
        status_data = {}
        # [ТУК СЕ ИЗВЪРШВА ЦЯЛОТО ПАРСВАНЕ НА status_raw с get_int_value и т.н.]
        
        # За удобство, ще включим само структурата:
        ...
        # ... (Парсване на останалите status полета) ...
        ...
        # ... (network_statistics парсинг) ...
        # (За опростяване тук ще прескочим мрежовата статистика, но в реалния скрипт я оставете!)
...
# --- Функция за Запис на JSON и TXT Файлове ---
...
def save_parsed_data(parsed_data):
    ...
    uptime_value = parsed_data["uptime_data"].get("uptime_duration_value", "N/A")
    ...
    try:
        ...
        print(f"[SUCCESS] Uptime TXT saved to: {txt_file_path}")
    except OSError as e:
