...
# --- (Останалата част от помощните функции и parse_server_status_to_json е същата) ---
# (За удобство, добавям отново пълния скрипт, започвайки от импортирането)
...
# --- НОВА СЕКЦИЯ: Запис във файл ---

def save_json_output(json_data, directories, base_filename="d2gs_status"):
    """Saves the JSON data to the specified directories."""
    
    # Генериране на времеви печат (timestamp)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    # Файл за лога (с времеви печат)
    log_filename = f"{base_filename}_{timestamp}.json"
    
    # Файл за уеб достъп (винаги презаписва, за да има най-актуална информация)
    web_filename = f"{base_filename}_latest.json"

    print("\n--- Запис на данни ---")
    
    for directory in directories:
        try:
            # 1. Създаване на директорията, ако не съществува
            os.makedirs(directory, exist_ok=True)
            
            # 2. Определяне на пълния път на файла
            if 'logs' in directory:
                file_path = os.path.join(directory, log_filename)
                print(f"Запис на лог файл в: {file_path}")
            else:
                file_path = os.path.join(directory, web_filename)
                print(f"Запис на УЕБ файл (презапис) в: {file_path}")
            
            # 3. Запис на JSON-а
            with open(file_path, 'w', encoding='utf-8') as f:
                f.write(json_data)
                
            print(f"Успешен запис в {directory}")
            
        except OSError as e:
            # Грешка при създаване на папка или запис
            print(f"Грешка при запис във {directory}: {e}")
            print(f"Проверете дали потребителят, изпълняващ скрипта, има права за запис в {directory}.")

# --- Актуализирано изпълнение на скрипта ---

if __name__ == "__main__":
    
    # 1. Изпълнение на парсера
    json_output = parse_server_status_to_json()
    
    # 2. Дефиниране на директориите
    # "/var/www/html/data/" е за уеб достъп (трябва да съществува)
    # "logs/" е относителна папка в текущата директория на скрипта (ще бъде създадена)
    output_directories = [
        "/var/www/html/data/", 
    ...
    # 4. Принтиране на изхода към конзолата (по избор, за дебъгване)
    # print("\n--- JSON Изход (Конзола) ---")
    # print(json_output)
