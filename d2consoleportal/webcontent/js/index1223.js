...
        'ASS': 'Assassin' 
    };
    const cleanName = shortName.toUpperCase().trim().replace(/\s/g, ''); // Премахва интервали за по-добро съвпадение
...
    // 1. Server Uptime (от server_uptime.txt - Текст)
    try {
...
    }
    
    // 2. PvPGN Uptime (от data/games.txt - XML)
    try {
...
// --- load games info (превежда класовете и добавя линк към charinfo.html) ---
...
            const className = translateClass(ch.Class); // Прилагане на превода
            table += `<tr>
